// HOME_DEFAKE_v1 (CRLF-safe reissue) — stops the Home dashboard from showing invented numbers.
...
//     Sample widgets remain in the gallery, tagged "sample".
// spark-home.html uses CRLF; this build normalizes for matching and restores CRLF on write.
// Run from repo root:  node apply-home-defake.cjs
const fs = require("fs");
const F = "spark-home.html";
const raw = fs.readFileSync(F, "utf8");
if (raw.includes("HOME_DEFAKE_v1")) { console.log("Already applied."); process.exit(0); }
const hadCRLF = /\r\n/.test(raw);
...
fs.writeFileSync("spark-home.backup-defake-" + stamp + ".html", raw);
const out = hadCRLF ? h.replace(/\n/g, "\r\n") : h;   // restore original EOL
fs.writeFileSync(F, out);
console.log("APPLIED HOME_DEFAKE_v1 (CRLF-safe)");
...
console.log("  Sample widgets kept in the gallery, tagged SAMPLE");
