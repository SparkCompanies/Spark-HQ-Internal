# Lives at .github/workflows/deploy-worker.yml in the repo.
# Deploys the worker to Cloudflare whenever anything under worker/ changes on main.
name: Deploy Worker
...
jobs:
  deploy:
    runs-on: ubuntu-latestvar __defProp = Object.defineProperty;
...
var worker_default = {
  async fetch(request, env) {
    ...
    if (url.pathname === "/intake/preview") {
      ...
      const out = { version: "preview-v32-merged", weekEnding: we, checkDateFolder: checkDate, note: "PREVIEW ONLY - reads OneDrive and extracts; writes nothing, touches no invoices" };
      let token;
    ...
    if (url.pathname === "/sf-picklist") {
      ...
      const ALLOWED = { "Account": ["Subdivision__c", "Division__c"], "bpats__Job__c": ["Subdivision__c"] };
      ...
      if (ALLOWED[obj].indexOf(fieldName) === -1) return json({ error: "field not allowed" }, 400, origin);
      try {
        const tok = await getSalesforceToken(env);
    ...
    if (url.pathname === "/sf-update-account-bu-bulk") {
      ...
      try {
        ...
        const records = ids.map(function(id) {
          return { attributes: { type: "Account" }, Id: id, Subdivision__c: bu === "" ? null : bu };
        });
    ...
    if (url.pathname === "/sf-update-account-bu") {
      ...
      if (bu.length > 120 || company.length > 120) return json({ error: "value too long" }, 400, origin);
      try {
        const tok = await getSalesforceToken(env);
        const at = tok.access_token || tok.accessToken || tok.token;
        const base = String(tok.instance_url || "").replace(/\/+$/, "");
        ...
        const r = await fetch(base + "/services/data/v59.0/sobjects/Account/" + acctId, {
          method: "PATCH",
          headers: { "Authorization": "Bearer " + at, "Content-Type": "application/json" },
          body: JSON.stringify(setCompany ? { Division__c: company === "" ? null : company, Subdivision__c: bu === "" ? null : bu } : { Subdivision__c: bu === "" ? null : bu })
        });
    ...
    if (url.pathname === "/terr-map-data") {
      ...
      try {
        ...
        const asoql = "SELECT Id, Name, Subdivision__c, Division__c, ShippingCity, ShippingState, ShippingPostalCode, BillingCity, BillingState, BillingPostalCode FROM Account WHERE Name != null LIMIT 20000";
    ...
    if (url.pathname === "/boards-sf-sync" && request.method === "POST") {
      ...
      try {
        ...
        const soql = "SELECT bpats__ATS_Candidate__r.Name, Status__c, bpats__Start_Date__c FROM bpats__Placement__c WHERE bpats__Start_Date__c = LAST_N_DAYS:" + days + " ORDER BY bpats__Start_Date__c DESC";
        const sf = await runSalesforceQueryAll(env, soql);
        ...
        (sf.records || []).forEach((r) => {
          ...
          if (n && !byName[n]) byName[n] = { status: r.Status__c || "", start: r.bpats__Start_Date__c || "" };
        });
        let matched = 0;
...
# Cloudflare Worker deploy config — lives at worker/wrangler.toml in the repo
name = "spark-hq-worker"          # MUST match the worker's exact name in the Cloudflare dashboard
main = "cloudworker.js"           # the worker file, sitting next to this toml in worker/
account_id = "PASTE_ACCOUNT_ID_HERE"  # Cloudflare dashboard -> Workers & Pages -> right sidebar "Account ID"
compatibility_date = "2024-01-01"

# CRITICAL: preserves the environment variables already set in the Cloudflare dashboard
# (Supabase keys, Xero client id/secret, Graph creds, SF creds, XERO_CC_*, etc).
# Without this, a wrangler deploy can wipe dashboard-set plaintext vars.
keep_vars = true
