...
__name2(getPipelineSnapshot, "getPipelineSnapshot");
...
__name2(getSalesforceToken, "getSalesforceToken");
function round2(n) {
...
__name2(runSalesforceQueryAll, "runSalesforceQueryAll");
...
__name2(runSalesforceQuery, "runSalesforceQuery");
...
__name2(describeSalesforce, "describeSalesforce");
...
__name2(invokeBedrock, "invokeBedrock");
...
async function getMyCalendar(env, email, days) {
  try {
    ...
    const events = (data.value || []).filter(function(e) {
...
__name2(getMyCalendar, "getMyCalendar");
...
__name2(getGraphToken, "getGraphToken");
...
__name2(fetchDirectory, "fetchDirectory");
...
__name2(xeroTokenExchange, "xeroTokenExchange");
var XERO_CC = {
...
async function xeroAccessForTenant(env, tenantId) {
  ...
  let conn = res.data[0];
  const isFresh = /* @__PURE__ */ __name2((c) => !!(c && c.access_token && c.access_expires_at && new Date(c.access_expires_at).getTime() - Date.now() > 12e4), "isFresh");
  if (isFresh(conn)) return { access_token: conn.access_token, tenant_id: tenantId };
  const doRefresh = /* @__PURE__ */ __name2(async (oldRefresh) => {
...
__name2(xeroAccessForTenant, "xeroAccessForTenant");
function sbxCors() {
...
__name2(sbxPicklists, "sbxPicklists");
var worker_default = {
  async fetch(request, env) {
    ...
    if (url.pathname === "/intake/preview") {
      ...
      const d = /* @__PURE__ */ new Date(we + "T00:00:00Z");
      ...
      for (const ent of entities) {
        const parts2 = ["2026 TC " + ent, checkDate].map(encodeURIComponent).join("/");
        try {
          const r = await fetch(G + "/root:/" + parts2 + ":/children", { headers: H });
          if (!r.ok) {
            out.folders[ent] = { status: r.status };
            continue;
          }
          const data = await r.json();
          ...
          out.folders[ent] = { error: String(e.message || e) };
        }
      }
      const getText = /* @__PURE__ */ __name(async (f) => {
        const r = f.dl ? await fetch(f.dl) : await fetch(G + "/items/" + f.id + "/content", { headers: H });
        return await r.text();
      }, "getText");
      const qp = /* @__PURE__ */ __name((s) => s.replace(/=\r?\n/g, "").replace(/=([0-9A-Fa-f]{2})/g, (_m, h) => String.fromCharCode(parseInt(h, 16))), "qp");
      const strip = /* @__PURE__ */ __name((s) => s.replace(/<[^>]+>/g, " "), "strip");
      ...
      for (const f of penskeFiles) {
        try {
          if (/\.(pdf|jpe?g|png|gif|webp)$/i.test(f.name)) {
            ...
            if (_bb.byteLength > 8e6) {
              out.penske.invoices.push({ file: f.name, error: "file too large" });
              continue;
            }
            const _u = new Uint8Array(_bb);
            let _s = "";
            for (let k = 0; k < _u.length; k++) _s += String.fromCharCode(_u[k]);
            const _b64 = btoa(_s);
            ...
            if (!_air.ok) {
              out.penske.invoices.push({ file: f.name, error: "Claude error: " + (_ai && _ai.error && _ai.error.message ? _ai.error.message : _air.status) });
              continue;
            }
            const _txt = (Array.isArray(_ai.content) ? _ai.content : []).filter((b) => b && b.type === "text").map((b) => b.text).join(" ");
            let _p = null;
            try {
              _p = JSON.parse(_txt.slice(_txt.indexOf("{"), _txt.lastIndexOf("}") + 1));
            } catch (e2) {
            }
            const _po = _p && _p.po ? "POR " + String(_p.po).replace(/\D/g, "") : null;
            const _ws = _p && Array.isArray(_p.workers) ? _p.workers : [];
            _ws.forEach((w) => {
              if (w && w.name) penskeRaw.push({ worker: String(w.name).trim(), po: _po, reg: Number(w.reg) || 0 });
            });
          ...
          let m;
          while (m = re.exec(body)) hoursByName[m[1].trim()] = Number(m[2]) || 0;
          ...
          out.penske.invoices.push({ file: f.name, error: String(e.message || e) });
        }
      }
      ...
      for (const f of paslinFiles) {
        ...
        try {
          ...
          if (buf.byteLength > 6e6) {
            out.paslin.extracted.push({ file: f.name, entity, error: "PDF too large (" + buf.byteLength + " bytes)" });
            continue;
          }
          ...
          const b64 = btoa(bin);
          const payload2 = { model: "claude-sonnet-4-6", max_tokens: 1024, messages: [{ role: "user", content: [
          ...
          const air = await fetch("https://api.anthropic.com/v1/messages", { method: "POST", headers: { "x-api-key": env.ANTHROPIC_API_KEY, "anthropic-version": "2023-06-01", "content-type": "application/json" }, body: JSON.stringify(payload2) });
          const ai = await air.json();
          if (!air.ok) {
            out.paslin.extracted.push({ file: f.name, entity, error: "Claude error: " + (ai && ai.error && ai.error.message ? ai.error.message : air.status) });
            continue;
          }
          ...
          out.paslin.extracted.push({ file: f.name, entity, error: String(e.message || e) });
        }
      }
      let roster = {};
      try {
        ...
        const mapByAcct = {};
        mapRows.forEach((mm) => {
          mapByAcct[mm.sf_account_name] = mm.xero_contact;
        });
      ...
      const canonName = /* @__PURE__ */ __name((s) => {
        const p = String(s).split(" ");
        if (p.length && NICKMAP[p[0]]) p[0] = NICKMAP[p[0]];
        return p.join(" ");
      }, "canonName");
      const lev = /* @__PURE__ */ __name((a, b) => {
        ...
        for (let i2 = 1; i2 <= m; i2++) {
          ...
          for (let j2 = 1; j2 <= n; j2++) {
            ...
            cur[j2] = Math.min(prev[j2] + 1, cur[j2 - 1] + 1, prev[j2 - 1] + c);
          }
          prev = cur;
        }
        return prev[n];
      }, "lev");
      const ratio = /* @__PURE__ */ __name((a, b) => {
      ...
      const aliasMap = {};
      try {
        const _al = await sbService(env, "GET", "fin_name_aliases?select=client,doc_name,action,sf_name");
        if (_al && _al.ok && Array.isArray(_al.data)) _al.data.forEach((a) => {
          aliasMap[String(a.client) + "|" + String(a.doc_name).toLowerCase()] = { action: a.action, sf_name: a.sf_name };
        });
      } catch (e) {
      }
      const matchOne = /* @__PURE__ */ __name((name, pool, clientKey) => {
        const dn = String(name).replace(/\s+/g, " ").trim();
        const _ali = clientKey ? aliasMap[clientKey + "|" + dn.toLowerCase()] : null;
        if (_ali) {
          if (_ali.action === "ignore") return { status: "ignored", sfName: null, nearest: null, score: 1 };
          if (_ali.action === "fix_sf") return { status: "fix_sf", sfName: null, nearest: null, score: 1 };
          if (_ali.action === "matched" && _ali.sf_name) return { status: "matched", sfName: _ali.sf_name, nearest: _ali.sf_name, score: 1, viaAlias: true };
        }
        const nk = canonName(dn.toLowerCase());
        let best = null, second = 0;
        for (const c of pool) {
          const cz = canonName(c.nz);
          const s = cz === nk ? 1 : ratio(nk, cz);
          if (!best || s > best.s) {
            second = best ? best.s : 0;
            best = { c, s };
          } else if (s > second) {
            second = s;
          }
        }
        let status = "unmatched", sfName = null;
        if (best && best.s >= 0.995) {
          status = "matched";
          sfName = best.c.name;
        } else if (best && best.s >= 0.85 && best.s - second >= 0.08) {
          status = "matched";
          sfName = best.c.name;
        } else if (best && best.s >= 0.7) {
          status = "ambiguous";
        }
        const _res = { status, sfName, nearest: best ? best.c.name : null, score: best ? Math.round(best.s * 100) / 100 : 0 };
        if (status !== "matched" && clientKey) out.needsReview.push({ client: clientKey, docName: dn, nearest: _res.nearest, score: _res.score });
        return _res;
      }, "matchOne");
      ...
      out.penske.matching = [];
      out.penske.droppedZeroHours = [];
      const penskeRows = [];
      for (const row of penskeRaw) {
        const r = matchOne(row.worker, penskePool, "penske");
        if (r.status !== "matched" && Number(row.reg) === 0) {
          out.penske.droppedZeroHours.push({ extracted: row.worker });
          continue;
        }
        out.penske.matching.push({ extracted: row.worker, matchedTo: r.sfName, score: r.score, status: r.status, nearest: r.status === "matched" ? void 0 : r.nearest });
      ...
      for (const row of paslinRaw) {
        ...
        out.paslin.matching.push({ extracted: row.worker, entity: row.entity, matchedTo: r.sfName, score: r.score, status: r.status, nearest: r.status === "matched" ? void 0 : r.nearest });
      ...
      for (const f of methodsFiles) {
        try {
          ...
          const buf = await resp.arrayBuffer();
          if (buf.byteLength > 6e6) {
            out.methods.error = "PDF too large (" + buf.byteLength + " bytes)";
            continue;
          }
          const bytes = new Uint8Array(buf);
          let bin = "";
          for (let k = 0; k < bytes.length; k++) bin += String.fromCharCode(bytes[k]);
          const b64 = btoa(bin);
          const payload2 = { model: "claude-sonnet-4-6", max_tokens: 256, messages: [{ role: "user", content: [
          ...
          const ai = await air.json();
          if (!air.ok) {
            out.methods.error = "Claude error: " + (ai && ai.error && ai.error.message ? ai.error.message : air.status);
            continue;
          }
          ...
          out.methods.error = String(e.message || e);
        }
      }
      ...
      for (const f of fanucFiles) {
        try {
          ...
          if (!wsResp.ok) {
            (out.fanuc.fileIssues = out.fanuc.fileIssues || []).push(f.name + ": worksheets " + wsResp.status);
            continue;
          }
          ...
          for (const ws of sheets) {
            ...
            for (let ri = 0; ri < Math.min(vals.length, 12); ri++) {
              ...
              if (rowTxt.some((c) => c.indexOf("full name") !== -1) && rowTxt.some((c) => c.indexOf("overtime") !== -1)) {
                pickedName = ws.name;
                pickedHdr = ri;
                pickedVals = vals;
                break;
          ...
          if (!pickedVals) {
            (out.fanuc.skipped = out.fanuc.skipped || []).push(f.name + " (no rate sheet - not the timesheet)");
            continue;
          }
          ...
          for (let ri = pickedHdr + 1; ri < pickedVals.length; ri++) {
            ...
            if (raw.indexOf(",") !== -1) {
              const p = raw.split(",");
              disp = p.slice(1).join(" ").trim() + " " + p[0].trim();
            }
            disp = disp.split(/\s+/).filter((t) => t.length > 1).join(" ");
            fanucContractors.push({ raw, name: disp, ot_rate: otRate === otRate ? otRate : null, reg_rate: regRate === regRate ? regRate : null });
          }
        } catch (e) {
          (out.fanuc.fileIssues = out.fanuc.fileIssues || []).push(f.name + ": " + String(e.message || e));
        }
      }
      ...
      for (const c of fanucContractors) {
        ...
        out.fanuc.matching.push({ extracted: c.raw, as: c.name, matchedTo: r.sfName, regRate: c.reg_rate, otRate: c.ot_rate, score: r.score, status: r.status, nearest: r.status === "matched" ? void 0 : r.nearest });
      ...
      for (const f of fanucExpFiles) {
        try {
          const wsResp = await fetch(G + "/items/" + f.id + "/workbook/worksheets", { headers: H });
          if (!wsResp.ok) {
            out.fanucExpenses.error = "worksheets " + wsResp.status;
            continue;
          }
          const sheets = (await wsResp.json()).value || [];
          ...
          for (const ws of sheets) {
            ...
            for (let ri = 0; ri < Math.min(vv.length, 8); ri++) {
              ...
              if (t.some((c) => c.indexOf("full name") !== -1) && t.some((c) => c.indexOf("invoice") !== -1) && t.some((c) => c.indexOf("due") !== -1)) {
                vals = vv;
                hdrRow = ri;
                break;
              }
          ...
          if (!vals) {
            out.fanucExpenses.error = "no expense sheet (Full Name + Invoice # + Due Employee) found";
            continue;
          }
          ...
          out.fanucExpenses.error = String(e.message || e);
        }
      }
      const fanucExpPool = roster["fanuc america corporation"] || [];
      for (const e of fanucExpRaw) {
        const r = matchOne(e.name, fanucExpPool, "fanuc");
        out.fanucExpenses.rows.push({ extracted: e.name, matchedTo: r.sfName, invoiceNo: e.invoiceNo, amount: e.amount, status: r.status, nearest: r.status === "matched" ? void 0 : r.nearest });
      }
      ...
      for (const rc of reconDefs) {
        const rfAll = files.filter((f2) => new RegExp(rc.key, "i").test(f2.name));
        const f = rfAll.find((x) => /\.pdf$/i.test(x.name)) || rfAll.find((x) => /\.eml$/i.test(x.name)) || rfAll[0];
        if (!f) {
          out.reconcile.rows.push({ client: rc.label, note: "no file this week" });
          continue;
        }
        try {
          const resp = f.dl ? await fetch(f.dl) : await fetch(G + "/items/" + f.id + "/content", { headers: H });
          ...
          if (/\.pdf$/i.test(f.name)) {
            const buf = await resp.arrayBuffer();
            if (buf.byteLength > 8e6) {
              out.reconcile.rows.push({ client: rc.label, file: f.name, error: "PDF too large" });
              continue;
            }
            const bytes = new Uint8Array(buf);
            let bin = "";
            ...
            if (pi >= 0) {
              ...
              if (he >= 0) {
                let e = raw.indexOf("\r\n--", he);
                if (e < 0) e = raw.length;
                const blob = raw.slice(he + 4, e).replace(/[\r\n\t ]/g, "");
                if (blob.length > 200) pdfB64 = blob;
              }
            }
            if (pdfB64) {
              ...
              let body = raw;
              const ti = raw.search(/content-type:\s*text\/(plain|html)/i);
              if (ti >= 0) {
                const he = raw.indexOf("\r\n\r\n", ti);
                if (he >= 0) {
                  let e = raw.indexOf("\r\n--", he);
                  if (e < 0) e = raw.length;
                  body = raw.slice(he + 4, e);
                }
              }
              body = body.replace(/=\r?\n/g, "").replace(/=([0-9A-Fa-f]{2})/g, (_m, h) => String.fromCharCode(parseInt(h, 16))).replace(/<[^>]+>/g, " ").slice(0, 6e3);
              content = [{ type: "text", text: rc.prompt + "\n\nHere is the emailed report text:\n" + body }];
            }
          } else {
            out.reconcile.rows.push({ client: rc.label, file: f.name, note: "unsupported format for total extraction" });
            continue;
          }
          ...
          const ai = await air.json();
          if (!air.ok) {
            out.reconcile.rows.push({ client: rc.label, file: f.name, error: "Claude " + (ai && ai.error && ai.error.message ? ai.error.message : air.status) });
            continue;
          }
          const txt = (Array.isArray(ai.content) ? ai.content : []).filter((b) => b && b.type === "text").map((b) => b.text).join(" ");
          ...
          out.reconcile.rows.push({ client: rc.label, file: f.name, error: String(e.message || e) });
        }
      }
      ...
      if (_dbg) {
        try {
          ...
          out.debugRates = (dr.records || []).filter((rec) => {
            const ts = rec.ASYMBL_Time__Timesheet__r || {};
            return String(ts.ASYMBL_Time__Candidate_Name__c || "").toLowerCase().indexOf(q) !== -1;
          }).map((rec) => {
            const ts = rec.ASYMBL_Time__Timesheet__r || {};
            const pl = ts.Placement__r || {};
            const job = pl.bpats__ATS_Job__r || {};
          ...
          out.debugRates = { error: String(e.message || e) };
        }
      }
      ...
      if (_dbgDFM) {
        try {
          ...
          out.debugDFM = { error: String(e.message || e) };
        }
      }
      ...
      if (commit) {
        ...
        try {
          ...
          if (out.fanucExpenses && Array.isArray(out.fanucExpenses.rows)) {
            ...
            if (expRows.length) {
              const fe = await sbService(env, "POST", "fin_weekly_intake", { week_ending: we, kind: "fanuc_expenses", rows: expRows });
              writes.push({ kind: "fanuc_expenses", ok: fe.ok, status: fe.status, rows: expRows.length });
            }
          }
          ...
          out.commitError = String(e.message || e);
          out.writes = writes;
        }
      } else {
    ...
    if (url.pathname === "/worker-profile") {
      ...
      const wpEsc = /* @__PURE__ */ __name2((s) => String(s == null ? "" : s).replace(/[&<>"']/g, (c) => ({
        "&": "&amp;",
        "<": "&lt;",
        ">": "&gt;",
        '"': "&quot;",
      ...
      const wpQuadLine = /* @__PURE__ */ __name2(() => {
        if (!wpQuad) return "";
      ...
      const wpRows = /* @__PURE__ */ __name2(() => WP_ORDER.map((k) => {
      ...
      try {
        ...
        const wpSend = /* @__PURE__ */ __name2(async (to, html) => {
    ...
    if (url.pathname === "/sf-board") {
      ...
      if (!jobId) {
        ...
        const diag = {};
        try {
          const pipeRes = await runSalesforceQueryAll(
            /* SF_BOARD_RUNNER_v2 */
            env,
            "SELECT SUM(Amount) amt FROM Opportunity WHERE IsClosed = false" + (bu ? " AND Account.Subdivision__c = '" + bu + "'" : "")
          );
          if (!pipeRes.ok) {
            diag.pipeline = pipeRes.error || "query not ok";
            ...
            diag.pipeline = "ok but empty/unaliased: " + JSON.stringify((pipeRes.records || [])[0] || null);
          }
        } catch (e) {
          diag.pipeline = "threw: " + String(e && e.message || e);
        }
        try {
          const weRes = await runSalesforceQueryAll(
            /* SF_BOARD_RUNNER_v2 */
            env,
          ...
          if (weRes.ok && weRes.records && weRes.records.length) {
            ...
            if (!hRes.ok) {
              ...
              diag.hours = "ok but no aggregate row returned";
            }
          } else if (!weRes.ok) {
            diag.week = weRes.error || "week query not ok";
          }
        } catch (e) {
          diag.hours = "threw: " + String(e && e.message || e);
        }
    ...
    if (url.pathname === "/ar-overdue") {
      ...
      const ent = url.searchParams.get("entity") || "Spark Talent";
      try {
        let due = /* @__PURE__ */ __name(function(inv) {
    ...
    if (url.pathname === "/pulse-sf") {
      ...
      try {
        ...
        const n = /* @__PURE__ */ __name2((v) => Math.round((Number(v) || 0) * 100) / 100, "n");
    ...
    if (url.pathname === "/sf-update-account-address") {
      ...
      try {
        ...
        try {
          ...
          if (Array.isArray(e2) && e2[0]) msg = (e2[0].errorCode ? e2[0].errorCode + " " : "") + (e2[0].message || "");
        } catch (x) {
        }
    ...
    if (url.pathname === "/sf-update-account-address-bulk") {
      ...
      if (ups.every(function(u) {
        return u && /^[a-zA-Z0-9]{15,18}$/.test(String(u.id || ""));
      ...
      try {
        ...
        const records = ups.map(function(u) {
          ...
          if (u.city !== void 0) rec.ShippingCity = String(u.city || "").trim() || null;
          if (u.state !== void 0) rec.ShippingState = String(u.state || "").trim().toUpperCase() || null;
          if (u.zip !== void 0) rec.ShippingPostalCode = String(u.zip || "").trim() || null;
          return rec;
        });
    ...
    if (url.pathname === "/terr-map-data") {
      ...
      if (!who.ok) return json({ error: who.reason || "Unauthorized" }, 401, origin);
      try {
        let st2 = /* @__PURE__ */ __name(function(v) {
          v = String(v || "").trim();
          if (!v) return "";
    ...
    if (url.pathname === "/terr-accounts") {
      ...
      if (!who.ok) return json({ error: who.reason || "Unauthorized" }, 401, origin);
      try {
        let norm = /* @__PURE__ */ __name(function(a) {
    ...
    if (url.pathname === "/terr-jobs") {
      ...
      if (!who.ok) return json({ error: who.reason || "Unauthorized" }, 401, origin);
      try {
        let realLL = /* @__PURE__ */ __name(function(j) {
          ...
          return null;
        }, "realLL"), zip5 = /* @__PURE__ */ __name(function(j) {
          const m = String(j.Postal_Code__c || "").match(/\b(\d{5})\b/);
          return m ? m[1] : "";
        }, "zip5");
        __name2(realLL, "realLL");
        __name2(zip5, "zip5");
    ...
    if (url.pathname === "/fin-aliases-save" && request.method === "POST") {
      ...
      const row = { client, doc_name: docName, action, sf_name: sfName, updated_at: (/* @__PURE__ */ new Date()).toISOString() };
      try {
    ...
    if (url.pathname === "/dh-batch") {
      ...
      try {
        ...
        const fmt = /* @__PURE__ */ __name2(function(d) {
    ...
    if (url.pathname === "/fin-batch") {
      ...
      try {
        ...
        const groups = {};
        const fmtDate = /* @__PURE__ */ __name2((d) => {
          if (!d) return "";
    ...
    if (url.pathname === "/sf-command") {
      ...
      if (!who.ok) return json({ error: who.reason || "Unauthorized" }, 401, origin);
      try {
        let buMap = /* @__PURE__ */ __name(function(data, idx) {
          const out = {};
          if (!data) return out;
          ...
          return out;
        }, "buMap"), grandOf = /* @__PURE__ */ __name(function(data, idx) {
    ...
    if (url.pathname === "/hc-snapshot") {
      ...
      try {
        let idxOf = /* @__PURE__ */ __name(function(data) {
          const aggs = data.reportMetadata && data.reportMetadata.aggregates || [];
        ...
        }, "idxOf"), grand = /* @__PURE__ */ __name(function(data, idx) {
        ...
        }, "grand"), byBU = /* @__PURE__ */ __name(function(data, idx) {
          const out = {};
          const groupings = data.groupingsDown && data.groupingsDown.groupings || [];
          const fm = data.factMap || {};
          ...
          return out;
        }, "byBU"), leafCounts = /* @__PURE__ */ __name(function(data, idx) {
          const fm = data.factMap || {};
          const out = {};
          (/* @__PURE__ */ __name2(/* @__PURE__ */ __name((function walk(groupings) {
            (groupings || []).forEach(function(g) {
          ...
          }), "walk"), "walk"))(data.groupingsDown && data.groupingsDown.groupings || []);
          return out;
        }, "leafCounts");
        __name2(idxOf, "idxOf");
        __name2(grand, "grand");
        __name2(byBU, "byBU");
        __name2(leafCounts, "leafCounts");
        const tok = await getSalesforceToken(env);
        async function runReport(id) {
          const r = await fetch(
            tok.instance_url + "/services/data/v60.0/analytics/reports/" + id + "?includeDetails=false",
    ...
    if (url.pathname === "/hc-clients") {
      ...
      const actualId = (url.searchParams.get("actual") || "00OV5000003HIoPMAW").replace(/[^a-zA-Z0-9]/g, "");
      try {
        let findClientCol = /* @__PURE__ */ __name(function(data) {
        ...
        }, "findClientCol"), clientCounts = /* @__PURE__ */ __name(function(data) {
        ...
        __name2(findClientCol, "findClientCol");
        __name2(clientCounts, "clientCounts");
        const tok = await getSalesforceToken(env);
    ...
    if (url.pathname === "/fin-dryrun") {
      ...
      let weekEnding = (url.searchParams.get("weekEnding") || "").trim();
      try {
        let HOLD = /* @__PURE__ */ __name(function(msg) {
          const e = new Error(msg);
          e.__hold = true;
          return e;
        }, "HOLD"), route = /* @__PURE__ */ __name(function(xero, p, workers, rule, staticPO) {
        ...
        }, "route"), splitRoute = /* @__PURE__ */ __name(function(xero, rule, workers, staticPO) {
        ...
        const nz = /* @__PURE__ */ __name2((s) => String(s == null ? "" : s).replace(/\s+/g, " ").trim().toLowerCase(), "nz");
        const m2 = /* @__PURE__ */ __name2((x) => round2(x), "m2");
        const fmtDate = /* @__PURE__ */ __name2((d) => {
          if (!d) return "";
        ...
        const addDays = /* @__PURE__ */ __name2((iso, n) => {
        ...
        (intakeRows || []).forEach((ir) => {
          ...
          else if (ir.kind === "fanuc_expenses") {
            DROP.fanuc_expenses = rows;
          } else if (ir.kind === "client_total") rows.forEach((r) => {
            if (r.contact) DROP.client_total[nz(r.contact)] = Number(r.total);
          });
        ...
        Object.keys(agg).forEach((key) => {
          ...
          try {
            ...
            const crKey = /* @__PURE__ */ __name2((n) => DROP.client_rates[nz(g.client) + "|" + nz(n)], "crKey");
            if (false) {
            ...
            if (prof.uses_client_timesheet) {
              ...
              const _hasRates = need.length > 0 && need.every((n) => crKey(n));
              if (!_hasTotal && !_hasRates) throw HOLD("invoice must reconcile to the client timesheet \u2014 provide it");
            }
        ...
        if (DROP.fanuc_expenses && DROP.fanuc_expenses.length) {
          ...
          if (fx) {
            const exLines = [];
            DROP.fanuc_expenses.forEach((e) => {
              const amt = m2(Number(e.amount));
              if (amt) exLines.push({ desc: e.worker + " Expenses - " + e.invoiceNo, hours: 1, rate: amt, amount: amt });
            });
            if (exLines.length) {
    ...
    if (url.pathname === "/match-search" && request.method === "POST") {
      ...
      const clean = /* @__PURE__ */ __name2((v) => String(v == null ? "" : v).replace(/['\\]/g, "").trim(), "clean");
      ...
      const miles = /* @__PURE__ */ __name2((a, b) => {
        if (!a || !b) return null;
    ...
    const tkET = (function() {
      ...
      return function(iso) {
        let v = cache.get(iso);
        if (v) return v;
        const parts2 = fmt.formatToParts(new Date(iso));
        const g = {};
        ...
        if (h === 24) h = 0;
        v = { d: g.year + "-" + g.month + "-" + g.day, h, t: new Date(iso).getTime() };
        if (cache.size > 2e4) cache.clear();
        cache.set(iso, v);
        return v;
      };
    })();
    const tkAddDays = /* @__PURE__ */ __name(function(ymd, n) {
      const p = ymd.split("-").map(Number);
      return new Date(Date.UTC(p[0], p[1] - 1, p[2] + n)).toISOString().slice(0, 10);
    }, "tkAddDays");
    const tkSb = /* @__PURE__ */ __name(async function(env2, pathQ) {
      const r = await fetch(env2.SUPABASE_URL + "/rest/v1/" + pathQ, {
        headers: {
    ...
    const tkDayHours = /* @__PURE__ */ __name(function(punches, exempt, pol) {
      const sorted = punches.slice().sort(function(a, b) {
        return tkET(a.time).t - tkET(b.time).t;
      });
      ...
      const grossH = gross / 36e5;
      const reqH = 8;
      const mins = 30;
      const breakMs = exempt ? 0 : grossH >= reqH ? mins * 6e4 : 0;
      return { grossH, netH: Math.max(0, (gross - breakMs) / 36e5), unpaired };
    }, "tkDayHours");
    const tkDayPunches = /* @__PURE__ */ __name(function(byEid, eid, ymd) {
      const all = byEid[eid] || [];
      let dp = all.filter(function(p) {
        return tkET(p.time).d === ymd;
      }).sort(function(a, b) {
        return tkET(a.time).t - tkET(b.time).t;
      });
      ...
      if (last && last.type === "in" && tkET(last.time).h >= 14) {
        ...
        const next = all.filter(function(p) {
          return tkET(p.time).d === nd && tkET(p.time).h < 12;
        }).sort(function(a, b) {
          return tkET(a.time).t - tkET(b.time).t;
        });
        for (let i = 0; i < next.length; i++) {
          dp.push(next[i]);
          if (next[i].type === "out") break;
        }
      }
      if (dp.length && dp[0].type !== "in") {
        ...
        if (priorEveningIn) {
          ...
          if (firstIn > 0) dp = dp.slice(firstIn);
            ...
            const evening = dp.filter(function(p) {
              return tkET(p.time).h >= 12;
            });
            dp = evening.length ? evening : [];
          }
        }
      }
      return dp.sort(function(a, b) {
        return tkET(a.time).t - tkET(b.time).t;
      });
    }, "tkDayPunches");
    const tkComputeWeek = /* @__PURE__ */ __name(async function(env2, periodEnd) {
      let pol = { otWeekly: 40, breakReqHrs: 8, breakMins: 30 };
      try {
        const pr = await tkSb(env2, "tk_config?key=eq.policy&select=value");
        if (Array.isArray(pr) && pr[0] && pr[0].value && typeof pr[0].value === "object") {
          pol = Object.assign(pol, pr[0].value);
        }
      } catch (e) {
      }
      const emps = await tkSb(
        env2,
        "tk_employees?select=id,badge,fn,ln,break_exempt,sf_placement_id&sf_placement_id=not.is.null&status=eq.active&order=ln.asc"
      );
      ...
      for (let off = 0; ; off += 1e3) {
        const page = await tkSb(env2, "tk_punches?select=eid,type,time&time=gte." + fromIso + "&time=lte." + toIso + "&order=time.asc&limit=1000&offset=" + off);
        for (let i = 0; i < page.length; i++) punches.push(page[i]);
        if (!Array.isArray(page) || page.length < 1e3) break;
      }
      const byEid = {};
      punches.forEach(function(p) {
        (byEid[p.eid] = byEid[p.eid] || []).push(p);
      });
      const days = [];
      for (let i = 0; i < 7; i++) days.push(tkAddDays(monday, i));
      ...
      const rows = emps.map(function(e) {
        ...
        const weekH = parseFloat(total.toFixed(2));
        return {
          eid: e.id,
          badge: e.badge,
          name: e.fn + " " + e.ln,
      ...
      return { rows, policy: { otWeekly: otW }, monday };
    }, "tkComputeWeek");
    const tkSfWeek = /* @__PURE__ */ __name(async function(env2, periodEnd, placementIds) {
      const out = { byPlacement: {}, entriesByTs: {} };
      if (!placementIds.length) return out;
      const pidList = "('" + placementIds.join("','") + "')";
      const ts = await runSalesforceQueryAll(
        env2,
        "SELECT Id, Placement__c, ASYMBL_Time__Status__c FROM ASYMBL_Time__Timesheet__c WHERE ASYMBL_Time__Pay_Period_End_Date__c = " + periodEnd + " AND Placement__c IN " + pidList
      );
      ...
      const tsIds = (ts.records || []).map(function(t) {
        return t.Id;
      });
      if (tsIds.length) {
        const en = await runSalesforceQueryAll(
          env2,
          "SELECT Id, ASYMBL_Time__Timesheet__c, ASYMBL_Time__AST_Sequence__c, ASYMBL_Time__Regular_Hours__c, ASYMBL_Time__Overtime_Hours__c FROM ASYMBL_Time__Time_Entry__c WHERE ASYMBL_Time__Timesheet__c IN ('" + tsIds.join("','") + "')"
        );
        ...
        (en.records || []).forEach(function(x) {
          ...
          else (slot.other = slot.other || []).push({ id: x.Id, seq });
        });
      }
      return out;
    }, "tkSfWeek");
    const tkNear = /* @__PURE__ */ __name(function(a, b) {
      return Math.abs(Number(a || 0) - Number(b || 0)) < 0.1001;
    }, "tkNear");
    const tkPlan = /* @__PURE__ */ __name(function(row, sf) {
      ...
      return Object.assign(base, { action: ops.length ? "write" : "none", ops });
    }, "tkPlan");
    if (url.pathname === "/tk-hours-preview") {
      ...
      if (!/^\d{4}-\d{2}-\d{2}$/.test(periodEnd)) {
        const todayEt = tkET((/* @__PURE__ */ new Date()).toISOString()).d;
        const wd = (/* @__PURE__ */ new Date(todayEt + "T00:00:00Z")).getUTCDay();
        periodEnd = tkAddDays(todayEt, -wd);
      }
      const dowChk = (/* @__PURE__ */ new Date(periodEnd + "T00:00:00Z")).getUTCDay();
      ...
      try {
        ...
        const sf = await tkSfWeek(env, periodEnd, wk.rows.map(function(r) {
        ...
        rows.forEach(function(r) {
          const k = r.action + (r.reason ? ":" + r.reason : "");
          summary[k] = (summary[k] || 0) + 1;
        });
        return json({
          periodEnd,
          weekOf: wk.monday,
          otWeekly: wk.policy.otWeekly,
          employees: rows.length,
          summary,
          rows,
          policyNote: "Lunch rule uses 8h/30m to match the TimeKeep app. tk_config.policy may say breakReqHrs 6, but index.html never applies it (window.__breakReqHrs is unassigned). Resolve that policy question before changing this.",
    ...
    if (url.pathname === "/tk-hours-push" && request.method === "POST") {
      ...
      if (!/^\d{4}-\d{2}-\d{2}$/.test(periodEnd) || (/* @__PURE__ */ new Date(periodEnd + "T00:00:00Z")).getUTCDay() !== 0)
      ...
      try {
        ...
        const dr = await fetch(
          tok.instance_url + "/services/data/v60.0/sobjects/ASYMBL_Time__Time_Entry__c/describe",
          { headers: { Authorization: "Bearer " + tok.access_token } }
        );
        const dd = await dr.json();
        const fmap = {};
        (dd.fields || []).forEach(function(f) {
          fmap[f.name] = f;
        });
        ...
        if (!fReg || !fReg.updateable || !fOt || !fOt.updateable) {
          return json({
            error: "Integration user cannot edit hours on ASYMBL_Time__Time_Entry__c. Grant edit access on Regular/Overtime Hours to the Connected App run-as user, then retry.",
            regUpdateable: !!(fReg && fReg.updateable),
            otUpdateable: !!(fOt && fOt.updateable)
          }, 403, origin);
        }
        ...
        wk.rows.forEach(function(r) {
          ...
          plan.ops.forEach(function(op) {
            if (op.kind === "update") {
              ...
              const rec = {
                attributes: { type: "ASYMBL_Time__Time_Entry__c" },
        ...
        const chunks = /* @__PURE__ */ __name(function(arr) {
          const o = [];
          for (let i = 0; i < arr.length; i += 200) o.push(arr.slice(i, i + 200));
          return o;
        }, "chunks");
        for (const ch of chunks(updates)) {
          const r = await fetch(
            tok.instance_url + "/services/data/v60.0/composite/sobjects",
            {
              method: "PATCH",
              headers: { Authorization: "Bearer " + tok.access_token, "Content-Type": "application/json" },
        ...
        return json({
          ...
          results,
          counts: {
            write: report.filter(function(x) {
              return x.action === "write";
            }).length,
            matched: report.filter(function(x) {
              return x.action === "none";
            }).length,
            skipped: report.filter(function(x) {
    ...
    if (url.pathname === "/tk-placements") {
      ...
      const q = cleaned.split(String.fromCharCode(39)).join(String.fromCharCode(92, 39));
      const soql = "SELECT Id, Name, Status__c, Generate_Timesheets__c, bpats__Start_Date__c, bpats__Estimated_End_Date__c, Terminated_Date__c, Termination_Reason__c, Job_Title__c, bpats__Pay_Rate__c, bpats__ATS_Candidate__c, bpats__ATS_Candidate__r.Name, bpats__ATS_Job__r.Name, bpats__ATS_Job__r.bpats__Account_Name__c FROM bpats__Placement__c WHERE bpats__ATS_Job__r.bpats__Account_Name__c LIKE '%DFM%' AND bpats__ATS_Candidate__r.Name LIKE '%" + q + "%' ORDER BY bpats__Start_Date__c DESC LIMIT 25";
      const res = await runSalesforceQueryAll(env, soql);
      if (!res.ok) return json({ error: res.error, soql }, 502, origin);
      const rows = (res.records || []).map(function(r) {
        ...
        return {
          ...
          payRate: r.bpats__Pay_Rate__c === void 0 ? null : r.bpats__Pay_Rate__c
        };
      });
      const seen = {};
      const dupes = [];
      rows.forEach(function(x) {
        ...
        if (!k) return;
        if (seen[k]) {
          if (dupes.indexOf(k) === -1) dupes.push(k);
        }
        seen[k] = true;
      });
      return json({
    ...
    if (url.pathname === "/tk-active-roster") {
      ...
      if (!/^\d{4}-\d{2}-\d{2}$/.test(periodEnd)) {
        const nowEt = new Date(
          (/* @__PURE__ */ new Date()).toLocaleString("en-US", { timeZone: "America/Detroit" })
        );
        nowEt.setDate(nowEt.getDate() + (7 - nowEt.getDay()) % 7);
        const mm = String(nowEt.getMonth() + 1);
        const dd = String(nowEt.getDate());
        periodEnd = nowEt.getFullYear() + "-" + (mm.length < 2 ? "0" + mm : mm) + "-" + (dd.length < 2 ? "0" + dd : dd);
      }
      const soql = "SELECT Id, Placement__c, ASYMBL_Time__Candidate_Name__c, ASYMBL_Time__Status__c, ASYMBL_Time__Pay_Period_End_Date__c, Placement__r.Name, Placement__r.Status__c, Placement__r.Generate_Timesheets__c, Placement__r.Terminated_Date__c, Placement__r.Job_Title__c, Placement__r.bpats__ATS_Candidate__c, Placement__r.bpats__ATS_Candidate__r.Name FROM ASYMBL_Time__Timesheet__c WHERE ASYMBL_Time__Pay_Period_End_Date__c = " + periodEnd + " AND Placement__r.bpats__ATS_Job__r.bpats__Account_Name__c LIKE '%DFM%' ORDER BY ASYMBL_Time__Candidate_Name__c";
      const res = await runSalesforceQueryAll(env, soql);
      if (!res.ok) return json({ error: res.error, soql }, 502, origin);
      const byPlacement = {};
      const noPlacement = [];
      ...
      (res.records || []).forEach(function(r) {
        ...
        if (!byPlacement[pid]) {
          byPlacement[pid] = {
            ...
            candidateName: p && p.bpats__ATS_Candidate__r && p.bpats__ATS_Candidate__r.Name || r.ASYMBL_Time__Candidate_Name__c || "",
    ...
    if (url.pathname === "/charge-dh") {
      ...
      if (!/^\d{4}-\d{2}-\d{2}$/.test(weekEnding)) return json({ error: "weekEnding=YYYY-MM-DD required" }, 400, origin);
      try {
        const r2s = /* @__PURE__ */ __name((n) => Math.round((Number(n) || 0) * 100) / 100, "r2s");
        const d0 = /* @__PURE__ */ new Date(weekEnding + "T12:00:00Z");
        ...
        const burdenFor = /* @__PURE__ */ __name((entity, company, employee, title) => {
        ...
        for (const T of TRACKERS) {
          try {
            ...
            for (let i = 0; i < Math.min(6, grid.length); i++) {
              if ((grid[i] || []).some((c) => String(c).trim() === "Company")) {
                hr = i;
                break;
              }
            }
            ...
            const col = /* @__PURE__ */ __name((re) => H2.findIndex((h) => re.test(h)), "col");
            ...
            for (let c = 0; c < H2.length; c++) {
              const h = grid[hr][c];
              if (typeof h === "number" && Math.round(h) === monSerial) {
                wc = c;
                break;
              }
              if (labelCandidates.indexOf(String(h).trim()) !== -1) {
                wc = c;
                break;
              }
            }
            if (wc < 0) {
              review.push({ company: T.entity, candidate: "(tracker)", flags: ["dh_week_col_missing"], credits: ["No column for Mon " + labelCandidates[0]] });
              continue;
            }
            for (let r = hr + 1; r < grid.length; r++) {
              ...
              for (let c = wc + 1; c < row.length; c++) {
                if (typeof row[c] === "number" && row[c] !== 0) aheadDrops++;
              }
              for (let c = 0; c <= wc; c++) {
                if (c >= 13 && typeof row[c] === "number" && row[c] !== 0 && String(grid[hr][c]).length <= 9) paidSoFar++;
              }
              ...
              let ofN = allDrops > 0 ? allDrops : m ? Number(m[1]) : mInv ? Number(mInv[1]) : 1;
              ...
              drops.push({
                ...
                employee: emp,
                title: ttl,
                sales_rep: cSr >= 0 ? String(row[cSr] || "").trim() : "",
                recruiter: cRc >= 0 ? String(row[cRc] || "").trim() : "",
                bu: cBu >= 0 ? String(row[cBu] || "").trim() : "",
                loc: cLoc >= 0 ? String(row[cLoc] || "").trim() : "",
                invoicing_type: invType,
                invoiced: r2s(v),
                burden,
                amount: r2s(v * (1 - burden)),
                drop: paidSoFar + " of " + ofN,
                remaining: Math.max(ofN - paidSoFar, 0),
                internal: !!ENTSET[co.toLowerCase().replace(/,? llc$/, "").trim()]
              });
            }
          } catch (e) {
            review.push({ company: T.entity, candidate: "(tracker)", flags: ["dh_tracker_error"], credits: [String(e.message || e).slice(0, 100)] });
          }
        }
        ...
        const okey = /* @__PURE__ */ __name((e, c, n) => (String(e || "") + "|" + String(c || "") + "|" + String(n || "")).toLowerCase(), "okey");
        drops = drops.filter((d) => {
          ...
          for (const o of hit) {
            if (o.action === "hide") {
              review.push({ company: d.company, candidate: d.employee, flags: ["dh_hidden"], credits: ["Hidden by editor" + (o.notes ? ": " + o.notes : "")] });
              return false;
            }
            if (o.action === "patch" && o.fields) {
              const f = o.fields;
              ["company", "employee", "sales_rep", "recruiter", "bu", "entity", "title"].forEach((k) => {
                if (f[k] !== void 0) d[k] = f[k];
              });
              if (f.sales_rep !== void 0 || f.recruiter !== void 0) d.credEdited = true;
              if (f.bu !== void 0) d.buEdited = true;
              if (f.amount !== void 0) {
                d.amount = r2s(f.amount);
                d.invoiced = null;
                d.burden = null;
                d.amtEdited = true;
              }
          ...
          if (d.edited) {
            d.m_entity = k0.entity;
            d.m_company = k0.company;
            d.m_employee = k0.employee;
          }
          return true;
        });
        const mR = await sbService(env, "GET", "charge_dh_schedule?select=*&source=eq.manual&status=eq.active");
        const monthsBetween = /* @__PURE__ */ __name((a, b) => (b.getUTCFullYear() - a.getUTCFullYear()) * 12 + (b.getUTCMonth() - a.getUTCMonth()), "monthsBetween");
        for (const s of mR.ok && mR.data || []) {
          const first = /* @__PURE__ */ new Date(s.first_we + "T12:00:00Z");
          const n = Number(s.installments) || 1, paid = Number(s.weeks_paid) || 0, total2 = Number(s.charge) || 0;
          let k = -1;
          if (s.interval === "month") {
            const m = monthsBetween(first, d0);
            const target = new Date(first.getTime());
            target.setUTCMonth(target.getUTCMonth() + m);
            if (m >= 0 && Math.abs(d0.getTime() - target.getTime()) / 864e5 <= 3) k = m;
          } else {
            const wks = Math.round((d0.getTime() - first.getTime()) / (7 * 864e5));
            if (wks >= 0 && (d0.getTime() - first.getTime()) % (7 * 864e5) === 0) k = wks;
          }
          if (k < 0 || k >= n - paid) continue;
          drops.push({ source: "manual", schedId: s.id, entity: s.entity, company: s.company, employee: s.employee, title: "", sales_rep: s.sales_rep || "", recruiter: s.recruiter || "", bu: s.bu || "", invoicing_type: s.interval, invoiced: null, burden: null, amount: r2s(total2 / n), internal: false, drop: paid + k + 1 + " of " + n });
        }
        try {
          ...
          const uniq = /* @__PURE__ */ __name((list) => list.length === 1 ? list[0].full : null, "uniq");
          const codeMap = {};
          roster.forEach((full) => {
            const t = full.trim().split(/\s+/);
            if (t.length >= 2) {
          ...
          const resolve = /* @__PURE__ */ __name((nm) => {
            ...
            const firstMatch = /* @__PURE__ */ __name((p) => p.first === f || p.first.indexOf(f) === 0 || f.indexOf(p.first) === 0, "firstMatch");
          ...
          drops.forEach((d) => {
            d.sales_rep = resolve(d.sales_rep);
            d.recruiter = resolve(d.recruiter);
          });
        } catch (e) {
        }
        drops.forEach((d) => {
          if (/^bolt/i.test(String(d.entity || "")) && !d.buEdited) d.bu = "Bolt Creative Strategies";
        });
        drops.forEach((d) => {
          if (/\bbpo\b/i.test(String(d.employee || "") + " " + String(d.title || "")) && !d.buEdited) d.bu = "BPO";
        });
        ...
        for (const t of terrR.ok && terrR.data || []) {
          ...
          const g = t.geo || {};
          (g.states || []).forEach((st) => {
            stateBU[String(st).toUpperCase()] = bu;
          });
        }
        const needState = drops.filter((d) => !d.bu && !d.internal);
        const locState = /* @__PURE__ */ __name((loc) => {
          const m = String(loc || "").match(/,\s*([A-Z]{2})\b/);
          return m ? m[1] : null;
        }, "locState");
        needState.forEach((d) => {
          const st = locState(d.loc);
          if (st && stateBU[st]) d.bu = stateBU[st];
        });
        ...
        if (missing.length) {
          try {
            ...
            const st2 = {};
            (q.ok && q.records || []).forEach((a) => {
              st2[a.Name.toLowerCase()] = (a.ShippingState || a.BillingState || "").toUpperCase().slice(0, 2);
            });
            needState.forEach((d) => {
              if (!d.bu) {
                const st = st2[String(d.company).toLowerCase()];
                if (st && stateBU[st]) d.bu = stateBU[st];
              }
            });
          } catch (e) {
        ...
        drops.forEach((d) => {
          if (!d.bu && !d.internal) review.push({ company: d.company, candidate: d.employee, flags: ["dh_no_bu"], credits: ["No BU: set in DH Editor or check account state"] });
        });
        const spR0 = await sbService(env, "GET", "charge_splits?select=*&active=eq.true");
        const splitRules0 = spR0.ok && spR0.data || [];
        const ruleFor = /* @__PURE__ */ __name((d) => splitRules0.find((sp) => {
        ...
        drops.forEach((d) => {
          if (ruleFor(d)) d.ruleOwned = true;
        });
        const entCanon = /* @__PURE__ */ __name((s) => String(s || "").toLowerCase().replace(/[.,]/g, "").replace(/\s+(llc|inc|co)$/i, "").trim(), "entCanon");
        const txtCanon = /* @__PURE__ */ __name((s) => String(s || "").toLowerCase().replace(/[^a-z0-9]/g, ""), "txtCanon");
        for (const pd of drops) {
          ...
          const own = drops.find((x) => x !== pd && !x.pairSplit && !x.split && !x.ruleOwned && !x.amtEdited && entCanon(x.entity) === ownerEnt && entCanon(pd.entity) !== ownerEnt && x.amount > pd.amount && (txtCanon(x.company).length >= 6 && ptxt.indexOf(txtCanon(x.company).slice(0, 10)) !== -1 || txtCanon(x.employee).length >= 6 && ptxt.indexOf(txtCanon(x.employee).slice(0, 8)) !== -1));
          if (!own) continue;
          const retained = r2s(own.amount - pd.amount);
          const hireBU = own.bu || pd.bu || "";
          if (hireBU) {
            if (!own.buEdited) own.bu = hireBU;
            if (!pd.buEdited) pd.bu = hireBU;
          }
        ...
        const addU = /* @__PURE__ */ __name((m, k, v) => {
          if (k) m[k] = r2s((m[k] || 0) + v);
        }, "addU");
        const addP = /* @__PURE__ */ __name((name, bucket, v) => {
          if (!name || /^house$/i.test(name)) return;
          const p = byPerson[name] || (byPerson[name] = { sales: 0, fd: 0, rec: 0, tt: 0 });
          p[bucket] = r2s(p[bucket] + v);
          p.tt = r2s(p.tt + v);
        }, "addP");
        const ruled = [];
        ...
        const sibCanon = /* @__PURE__ */ __name((s) => String(s || "").toLowerCase().replace(/[^a-z0-9]/g, ""), "sibCanon");
        const isOurEntity = /* @__PURE__ */ __name((co) => {
          const c = sibCanon(co);
          if (!c) return false;
        ...
        drops = drops.filter((d) => {
          ...
          if (twin) {
            review.push({ company: d.company, candidate: d.employee, flags: ["dh_split_suppressed"], credits: ["Slice of " + twin.company + " \u2014 the split rule already allocates 100% of that deal"] });
            return false;
          }
        ...
        for (const d of drops) {
          if (d.split) {
            ...
            if (sr && rc && sr.toLowerCase() !== rc.toLowerCase()) {
              addP(sr, "sales", d.amount);
              addP(rc, "rec", d.amount);
        ...
        try {
          ...
          if (sf.ok) {
            ...
            const canon = /* @__PURE__ */ __name((s) => String(s || "").toLowerCase().replace(/[^a-z]/g, ""), "canon");
        ...
        for (const d of drops) {
          ...
          allocs.forEach((a) => {
            ...
            if (!pct) return;
            const who2 = a.person || "House";
            const bucket = a.bucket === "sales" || a.bucket === "rec" ? a.bucket : "fd";
            shown.push(Object.assign({}, d, {
              entity: a.entity || d.entity,
              bu: a.bu || d.bu,
              company: d.company,
              employee: d.employee + " \xB7 " + pct + "%",
              sales_rep: bucket === "rec" ? "House" : who2,
              recruiter: bucket === "sales" ? "House" : who2,
        ...
        return json({ error: "dh-batch failed: " + String(e.message || e) }, 502, origin);
      }
    }
    if (url.pathname === "/dh-override") {
      ...
      if (who.ok !== true) return json({ error: who.reason || "Unauthorized" }, 401, origin);
      if (request.method === "GET") {
        const r2 = await sbService(env, "GET", "charge_dh_overrides?select=*&order=edited_at.desc&limit=300");
        return json({ ok: r2.ok, rows: r2.data }, r2.ok ? 200 : 502, origin);
      }
      const MAP_ADMINS = ["aspegel@sparkcompanies.com", "mpatrico@sparkcompanies.com", "pmalani@sparkcompanies.com", "aopalewski@sparkcompanies.com", "eurisitti@sparkcompanies.com", "bnamma@sparkcompanies.com", "bnaama@sparkcompanies.com"];
      let email = String(who.email || who.user && who.user.email || "").toLowerCase();
      if (email === "") {
      ...
      if (body.op === "delete" && body.id) {
        const r2 = await sbService(env, "DELETE", "charge_dh_overrides?id=eq." + Number(body.id));
        return json({ ok: r2.ok }, r2.ok ? 200 : 502, origin);
      }
      if (body.op === "revert") {
        ...
        const r2 = await sbService(env, "DELETE", q);
        return json({ ok: r2.ok }, r2.ok ? 200 : 502, origin);
      }
    ...
    if (url.pathname === "/charge-people") {
      ...
      if (who.ok !== true) return json({ error: who.reason || "Unauthorized" }, 401, origin);
      if (request.method === "GET") {
        const r2 = await sbService(env, "GET", "charge_people?select=person,role,entity,bu,active&order=person.asc");
        return json({ ok: r2.ok, rows: r2.data }, r2.ok ? 200 : 502, origin);
      }
      const MAP_ADMINS = ["aspegel@sparkcompanies.com", "mpatrico@sparkcompanies.com", "pmalani@sparkcompanies.com", "aopalewski@sparkcompanies.com", "eurisitti@sparkcompanies.com", "bnamma@sparkcompanies.com", "bnaama@sparkcompanies.com"];
      let email = String(who.email || who.user && who.user.email || "").toLowerCase();
      ...
      if (body.op === "deactivate" || body.op === "activate") {
        const r2 = await sbService(env, "PATCH", "charge_people?person=eq." + encodeURIComponent(person), { active: body.op === "activate" });
        return json({ ok: r2.ok, row: r2.data && r2.data[0] }, r2.ok ? 200 : 502, origin);
      }
      const row = { person };
      ["bu", "entity", "role"].forEach((k) => {
        if (body[k] !== void 0) row[k] = body[k];
      });
      ...
      if (r.ok && body.targets && typeof body.targets === "object") {
        ...
        if (unit) {
          ...
          Object.keys(map).forEach((k) => {
            ...
            if (isFinite(v) && v > 0) trows.push({ person, unit, period: map[k][0], kind: map[k][1], amount: v });
          });
    ...
    if (url.pathname === "/contract-override") {
      ...
      if (body.op === "edit_add" && body.id) {
        const r2 = await sbService(env, "PATCH", "charge_contract_overrides?id=eq." + Number(body.id), { fields: body.fields || {} });
        return json({ ok: r2.ok }, r2.ok ? 200 : 502, origin);
      }
      if (body.op === "delete_add" && body.id) {
        const r2 = await sbService(env, "DELETE", "charge_contract_overrides?id=eq." + Number(body.id));
        return json({ ok: r2.ok }, r2.ok ? 200 : 502, origin);
      }
    ...
    if (url.pathname === "/charge-week-lock") {
      ...
      const rec = { week_ending: wk, locked: body.locked !== false, locked_by: email, locked_at: (/* @__PURE__ */ new Date()).toISOString() };
    ...
    if (url.pathname === "/charge-credit-fix") {
      ...
      try {
        const ra = await sbService(env, "GET", "charge_name_aliases?select=sf_name,display_name");
        if (ra && ra.ok && Array.isArray(ra.data)) ra.data.forEach((m) => {
          if (m.display_name && m.sf_name) revAlias[m.display_name] = m.sf_name;
        });
      } catch (e) {
      }
      const toSF = /* @__PURE__ */ __name((nm) => revAlias[nm] || nm, "toSF");
      let localOk = false;
      try {
        ...
        try {
          await sbService(env, "DELETE", "charge_contract_overrides?week_ending=is.null&action=eq.patch&match_candidate=eq." + encodeURIComponent(rec.match_candidate) + "&match_company=eq." + encodeURIComponent(rec.match_company));
        } catch (e) {
        }
      ...
      if (placementId && body.write_sf !== false) {
        ...
        try {
          ...
          const q = tsId ? "SELECT Id FROM bpats__Placement_Credit__c WHERE Timesheet_Timesheet__c = '" + tsId + "' AND bpats__Is_Void__c = false" : "SELECT Id FROM bpats__Placement_Credit__c WHERE bpats__Placement__c = '" + pid + "' AND bpats__Is_Void__c = false";
          ...
          const mkRec = /* @__PURE__ */ __name((name, type, userId) => {
            const o = { Name: type, bpats__Placement__c: placementId, bpats__ATS_Role_Type__c: "User Lookup", bpats__Credit_Recipient__c: toSF(name), bpats__Credit_Percentage__c: 100 };
            if (userId) o.bpats__User__c = userId;
            if (tsId) o.Timesheet_Timesheet__c = tsId;
            return o;
          }, "mkRec");
          ...
          for (const rec of records) {
            ...
            if (cr.ok) created++;
            else errs.push(JSON.stringify(cr.data).slice(0, 140));
          }
          sf.deleted = deleted;
          sf.created = created;
          sf.timesheet = tsId || null;
          sf.ok = errs.length === 0 && created === records.length;
    ...
    if (url.pathname === "/sv7-sync") {
      ...
      try {
        ...
        return json({ error: "Auth check failed" }, 401, origin);
      }
      ...
      try {
        const sbAll = /* @__PURE__ */ __name(async (path) => {
          let out = [], off = 0;
          for (; ; ) {
        ...
        return json({ ok: true, syncedBy: em, at: (/* @__PURE__ */ new Date()).toISOString(), personWeeks, people, dh }, 200, origin);
      } catch (e) {
    ...
    if (url.pathname === "/dh-schedule") {
      ...
      if (who.ok !== true) return json({ error: who.reason || "Unauthorized" }, 401, origin);
      if (request.method === "GET") {
        const r2 = await sbService(env, "GET", "charge_dh_schedule?select=*&order=created_at.desc&limit=500");
        return json({ ok: r2.ok, rows: r2.data }, r2.ok ? 200 : 502, origin);
      }
      const MAP_ADMINS = ["aspegel@sparkcompanies.com", "mpatrico@sparkcompanies.com", "pmalani@sparkcompanies.com", "aopalewski@sparkcompanies.com", "eurisitti@sparkcompanies.com", "bnamma@sparkcompanies.com", "bnaama@sparkcompanies.com"];
      let email = String(who.email || who.user && who.user.email || "").toLowerCase();
      ...
      if (!id) return json({ error: "id required" }, 400, origin);
      const patch = {};
      ["interval", "installments", "weeks_paid", "first_we", "status", "burden_pct", "invoicing", "charge", "sales_rep", "recruiter", "entity", "bu", "company", "employee", "notes"].forEach((k) => {
        if (body[k] !== void 0) patch[k] = body[k];
      });
      if (patch.invoicing !== void 0 && patch.charge === void 0) {
        const b = patch.burden_pct !== void 0 ? Number(patch.burden_pct) : null;
        if (b !== null) patch.charge = Math.round(Number(patch.invoicing) * (1 - b) * 100) / 100;
      }
    ...
    if (url.pathname === "/dh-probe") {
      const who = await verifyUser(request, env);
      if (!who.ok) return json({ error: who.reason || "Unauthorized" }, 401, origin);
      const out = { ok: true, at: (/* @__PURE__ */ new Date()).toISOString() };
      ...
      try {
        ...
        detCols.forEach((c) => {
          cols[c] = extended[c] && extended[c].label || c;
        });
        out.sfReport = {
          name: meta.name,
          reportType: meta.reportType && meta.reportType.type,
          detailColumns: cols,
        ...
        for (const k of Object.keys(fm)) {
          if (fm[k] && Array.isArray(fm[k].rows) && fm[k].rows.length) {
            rows = fm[k].rows;
            break;
          }
        }
        ...
        out.sfError = String(e.message || e);
      }
      const TRACKERS = [
      ...
      try {
        ...
        out.graphError = String(e.message || e);
      }
      return json(out, 200, origin);
    }
    if (url.pathname === "/charge-history") {
      ...
      try {
        ...
        unitWeeks.forEach((r) => {
          if (!lastImported || r.week_ending > lastImported) lastImported = r.week_ending;
        });
        let locks = [];
        try {
          const lr = await sbService(env, "GET", "charge_week_locks?select=week_ending,locked,locked_by,locked_at");
          if (lr.ok && Array.isArray(lr.data)) locks = lr.data;
        } catch (e) {
        }
    ...
    if (url.pathname === "/charge-snapshot") {
      ...
      try {
        ...
        let b = posted && posted.batch && posted.batch.ok ? posted.batch : null;
        if (!b) {
        ...
        const bucket = /* @__PURE__ */ __name((t) => {
          t = String(t || "").toLowerCase();
          if (t.indexOf("full") !== -1) return "fd";
          if (t.indexOf("recruit") !== -1) return "rec";
          if (t.indexOf("account") !== -1 || t.indexOf("sales") !== -1) return "sales";
          return null;
        }, "bucket");
        const ppl = {};
        (b.rows || []).forEach((row) => {
          ...
          (row.credits || []).forEach((c) => {
            ...
            const bk = bucket(c.type);
            if (bk) p[bk] += row.charge;
          });
        });
        ...
        try {
          ...
          if (dj && dj.ok) {
            ...
            Object.keys(dj.byBU || {}).forEach((u) => unitRows.push({ week_ending: weekEnding, unit: u + " \xB7 DH", kind: "direct", charge: dj.byBU[u] }));
            Object.keys(dj.byPerson || {}).forEach((n) => {
              const p = dj.byPerson[n];
              let row = personRows.find((x) => x.person === n);
              if (!row) {
                row = { week_ending: weekEnding, person: n, sales: 0, fd: 0, rec: 0, tt: 0, raw: null };
                personRows.push(row);
              }
              row.sales = r2s(row.sales + p.sales);
              row.fd = r2s(row.fd + p.fd);
              row.rec = r2s(row.rec + p.rec);
              row.tt = r2s(row.tt + p.tt);
            });
            for (const d of dj.drops || []) {
              if (!d || d.id == null) continue;
              try {
                await sbService(env, "PATCH", "charge_dh_schedule?id=eq." + d.id, { weeks_paid: Number(String(d.drop || "").split(" ")[0]) || 0, status: (Number(d.remaining) || 0) <= 0 ? "done" : "active" });
              } catch (e2) {
              }
            }
            try {
        ...
        personRows.forEach((row) => {
          row.raw = r2s((Number(row.fd) || 0) + ((Number(row.sales) || 0) + (Number(row.rec) || 0)) / 2);
        });
    ...
    if (url.pathname === "/charge-batch") {
      ...
      try {
        ...
        try {
          ...
          if (r && r.ok && Array.isArray(r.data)) r.data.forEach((m) => {
            clientMap[m.sf_account_name] = { company: m.xero_contact || m.sf_account_name, entity: m.invoicing_entity || null };
          });
        } catch (e) {
        }
        const rateRules = {};
        try {
          ...
          if (r && r.ok && Array.isArray(r.data)) r.data.forEach((m) => {
            rateRules[m.sf_account_name] = { mode: m.rate_mode || "unset", ot: m.ot_multiplier, dt: m.dt_multiplier };
          });
        } catch (e) {
        }
        const rrKey = /* @__PURE__ */ __name((s) => String(s || "").toLowerCase().replace(/[^a-z0-9 ]/g, " ").replace(/\b(corporation|corp|incorporated|inc|llc|company|co)\b/g, "").replace(/\s+/g, " ").trim(), "rrKey");
        const rrNorm = {};
        Object.keys(rateRules).forEach((k) => {
          rrNorm[rrKey(k)] = rateRules[k];
        });
        const workerRates = {};
        try {
          const wrR = await sbService(env, "GET", "fin_worker_ot_rates?select=sf_account_name,candidate,ot_bill,dt_bill");
          if (wrR.ok && Array.isArray(wrR.data)) wrR.data.forEach((m) => {
            workerRates[rrKey(m.sf_account_name) + "|" + String(m.candidate || "").trim().toLowerCase()] = m;
          });
        } catch (e) {
        }
        try {
        ...
        try {
          ...
          if (r && r.ok && Array.isArray(r.data)) r.data.forEach((m) => {
            recipAlias[m.sf_name] = m.display_name;
          });
        } catch (e) {
        }
        ...
        const r2 = /* @__PURE__ */ __name((n) => Math.round((Number(n) || 0) * 100) / 100, "r2");
        const r4 = /* @__PURE__ */ __name((n) => Math.round((Number(n) || 0) * 1e4) / 1e4, "r4");
        for (const ts of tsRes.records || []) {
          ...
          if (map) dbg.mappedAccounts++;
          else dbg.unmappedAccounts[acct] = (dbg.unmappedAccounts[acct] || 0) + 1;
          ...
          let kept;
          if (sales.length && recr.length) {
            kept = [sales[0], recr[0]];
            if (credits.length > 2) flags.push("extra_credits");
          } else if (full.length) {
            kept = [full[0]];
            if (sales.length) kept.push(sales[0]);
            if (credits.length > kept.length) flags.push("extra_credits");
          } else if (credits.length === 1 && house.length === 1) {
            kept = credits;
            flags.push("lone_house");
          } else if (credits.length === 0) {
            kept = [];
            flags.push("no_credits");
          } else {
            kept = credits;
            if (credits.length > 2) flags.push("extra_credits");
          }
          ...
          rows.push({
            tsId: ts.Id,
            placementId: ts.Placement__c,
            jobId: pl.bpats__ATS_Job__c || null,
            company,
            entity,
            bu,
            title,
        ...
        if (url.searchParams.get("payables") !== "0") {
          ...
          try {
            ...
            const parts2 = weekEnding.split("-");
            const short = Number(parts2[1]) + "-" + Number(parts2[2]) + "-" + parts2[0].slice(2);
            const normName = /* @__PURE__ */ __name((s) => String(s).trim().toLowerCase().replace(/^we\s+/, ""), "normName");
            const target = (wd.value || []).map((x) => x.name).find((n) => normName(n) === short) || null;
            payables.sheet = target || null;
            if (!target) {
              payables.note = "No sheet named '" + short + "' or 'WE " + short + "' yet.";
            } else {
              ...
              const findCell = /* @__PURE__ */ __name((re) => {
                for (let r = 0; r < grid.length; r++) {
                  const row = grid[r] || [];
                  for (let c = 0; c < row.length; c++) {
                    if (re.test(String(row[c] || "").trim())) return { r, c };
                  }
                }
                return null;
              }, "findCell");
              const findHeader = /* @__PURE__ */ __name((anchor) => {
              ...
              const labelCol = /* @__PURE__ */ __name((rowIdx, fromC, re, span) => {
                const row = grid[rowIdx] || [];
                for (let c = fromC; c < Math.min(row.length, fromC + (span || 10)); c++) {
                  if (re.test(String(row[c] || "").trim().toLowerCase())) return c;
                }
                return null;
              }, "labelCol");
              const canon = /* @__PURE__ */ __name((s) => String(s || "").toLowerCase().replace(/[^a-z ]/g, " ").replace(/\s+/g, " ").trim(), "canon");
              const lev = /* @__PURE__ */ __name((a, b) => {
                const m = a.length, n = b.length;
                if (!m) return n;
                if (!n) return m;
              ...
              const matchRow = /* @__PURE__ */ __name((name, client) => {
                const nk = canon(name);
                if (!nk) return -1;
              ...
              const readNames = /* @__PURE__ */ __name((hdr, cb) => {
                for (let r = hdr.r + 1; r < grid.length; r++) {
                  const row = grid[r] || [];
                  const name = String(row[hdr.cName] || "").trim();
                  if (!name) {
                    const nxt = String((grid[r + 1] || [])[hdr.cName] || "").trim();
                    if (!nxt) break;
                    else continue;
                  }
              ...
              if (mH) {
                ...
                if (cReg == null && cW != null) {
                  cReg = cW + 1;
                  cOt = cW + 2;
                  cDt = cW + 3;
                }
                if (cReg != null) readNames(mH, (row, name) => {
                  const n = /* @__PURE__ */ __name((c) => {
                    const v = c != null ? row[c] : null;
              ...
              vacEntries.forEach((e) => {
                ...
                const r = rows[i];
                r.vacHrs = r2((r.vacHrs || 0) + e.hours);
                if (r.flags.indexOf("vacation") === -1) r.flags.push("vacation");
              ...
              for (const e of vacOnly.slice(0, 10)) {
                ...
                try {
                  ...
                  if (toks.length >= 2) {
                    ...
                    if (q.ok && q.records && q.records.length) {
                      ...
                      let burden = Number(pick.bpats__Burden_Percentage__c) || 0;
                      if (burden > 1) burden = burden / 100;
                      const vacM = -(bill - pay) * (1 + burden);
                      ...
                      rows.push({
                        tsId: null,
                        placementId: pick.Id,
                        jobId: pick.bpats__ATS_Job__c || null,
                        company: map ? map.company : acct,
                        entity: pick.Division__c || map && map.entity || null,
                        bu: pick.bpats__ATS_Job__r && pick.bpats__ATS_Job__r.Subdivision__c || null,
                        title,
                        account: acct,
                        candidate: pick.bpats__Candidate__r && pick.bpats__Candidate__r.Name || e.name,
                        pay: r2(pay),
                        otPay: r2(pay * 1.5),
                        dtPay: r2(pay * 2),
                        bill: r2(bill),
                        ...
                        vacHrs: e.hours,
                        totalHrs: 0,
                        regMargin: r4(bill - pay * (1 + burden)),
                        otMargin: 0,
                        dtMargin: 0,
                        vacMargin: r4(vacM),
                        charge: r2(e.hours * vacM),
                        credits: [],
                        flags: ["vacation", "vacation_only"]
                      });
                ...
                if (!added) {
                  payables.vacation.unmatched.push(e);
                  review.push({ tsId: null, placementId: null, company: e.client || "(payables)", candidate: e.name, flags: ["vac_unmatched"], credits: ["Vacation " + e.hours + "h \u2014 no match found"] });
                }
              }
              ...
              manEntries.forEach((e) => {
                const i = matchRow(e.name, e.client);
                if (i < 0) {
                  payables.manualPay.unmatched.push(e);
                  review.push({ tsId: null, placementId: null, company: e.client || "(payables)", candidate: e.name, flags: ["manual_unmatched"], credits: ["Manual pay " + (e.reg + e.ot + e.dt) + "h \u2014 no timesheet match"] });
                  return;
                }
                const r = rows[i];
                r.regHrs = r2(r.regHrs + e.reg);
                r.otHrs = r2(r.otHrs + e.ot);
                r.dtHrs = r2(r.dtHrs + e.dt);
                r.totalHrs = r2(r.totalHrs + e.reg + e.ot + e.dt);
                r.manualReg = r2((r.manualReg || 0) + e.reg);
                r.manualOt = r2((r.manualOt || 0) + e.ot);
                r.manualDt = r2((r.manualDt || 0) + e.dt);
                if (r.flags.indexOf("manual_pay") === -1) r.flags.push("manual_pay");
                payables.manualPay.matched++;
              });
              rows.forEach((r) => {
                r.charge = r2(r.regHrs * r.regMargin + r.otHrs * r.otMargin + r.dtHrs * r.dtMargin + (r.vacHrs || 0) * r.vacMargin);
              });
            }
          } catch (e) {
            payables.error = String(e.message || e);
          }
          try {
            const kR = await sbService(env, "GET", "charge_weekly_constants?select=*&active=eq.true");
            (kR.ok && kR.data || []).forEach((k) => {
              rows.push({
                entity: k.entity || "Spark Companies",
                company: k.company || "",
                bu: k.bu || "",
                ...
                dtMult: null,
                burden: 0,
                regHrs: Number(k.hours) || 0,
                otHrs: 0,
                dtHrs: 0,
                vacHrs: 0,
                totalHrs: Number(k.hours) || 0,
                regMargin: 0,
                otMargin: 0,
                dtMargin: 0,
                vacMargin: 0,
        ...
        try {
          ...
          const ovs = ovR.ok && ovR.data || [];
          if (ovs.length) {
            const ck = /* @__PURE__ */ __name((s) => String(s || "").trim().toLowerCase(), "ck");
            const exA = /* @__PURE__ */ __name((cr) => {
              for (const c of cr || []) {
                const t = String(c.type || "").toLowerCase();
                if (t.indexOf("account") !== -1 || t.indexOf("sales") !== -1 || t.indexOf("full") !== -1) return c.recipient || "";
              }
              return cr && cr[0] && cr[0].recipient || "";
            }, "exA");
            const exR = /* @__PURE__ */ __name((cr) => {
              for (const c of cr || []) {
                const t = String(c.type || "").toLowerCase();
                if (t.indexOf("recruit") !== -1 || t.indexOf("full") !== -1) return c.recipient || "";
              }
              return cr && cr[1] && cr[1].recipient || "";
            }, "exR");
            const mkCred = /* @__PURE__ */ __name((am, rc) => am && ck(am) === ck(rc) ? [{ recipient: am, type: "Full Desk" }] : [{ recipient: am, type: "Account Manager" }, { recipient: rc, type: "Recruiter" }], "mkCred");
            ovs.forEach((o) => {
            ...
            for (let i = rows.length - 1; i >= 0; i--) {
              ...
              for (const o of hit) {
                ...
                if (o.action === "patch" && o.fields) {
                  ...
                  ["bu", "entity", "title", "company", "candidate"].forEach((k) => {
                    if (f[k] !== void 0) r[k] = f[k];
                  });
        ...
        const roll = /* @__PURE__ */ __name((keyFn) => {
          const m = {};
          rows.forEach((r) => {
    ...
    if (url.pathname === "/charge-probe") {
      ...
      try {
        ...
        try {
          ...
          out.salesforce.reportDescribeError = String(e.message || e);
        }
        try {
          ...
          if (!r.ok) {
            ...
            out.salesforce.allData = d.allData;
            const cols = (out.salesforce.detailColumns || []).map(function(c) {
              return c.label || c.key;
            });
            out.salesforce.sampleRows = rows.slice(0, 3).map(function(row) {
              const o = {};
              (row.dataCells || []).forEach(function(cell, i) {
                o[cols[i] || "col" + i] = cell.label;
              });
              return o;
            });
          }
        } catch (e) {
          out.salesforce.reportRunError = String(e.message || e);
        }
        ...
        try {
          ...
          if (!r.ok) {
            ...
            out.salesforce.placementBurdenFields = (d.fields || []).filter(function(f) {
              return /burden/i.test(f.name) || /burden/i.test(f.label || "");
            }).map(function(f) {
              return { name: f.name, label: f.label, type: f.type };
            });
            out.salesforce.placementRateFields = (d.fields || []).filter(function(f) {
              return /pay_rate|bill_rate|overtime|double/i.test(f.name);
            }).map(function(f) {
              return { name: f.name, label: f.label, type: f.type };
            });
            ...
            out.salesforce.placementChildObjectsAll = (d.childRelationships || []).map(function(c) {
              return c.childSObject;
            }).slice(0, 80);
          }
        } catch (e) {
          out.salesforce.placementDescribeError = String(e.message || e);
        }
        if (creditObjectName) {
          try {
            ...
            if (!r.ok) {
              ...
              out.salesforce.creditObject = {
                ...
                fields: (d.fields || []).map(function(f) {
                  ...
                  const pv = (f.picklistValues || []).filter(function(p) {
                    return p.active;
                  }).map(function(p) {
                    return p.value;
                  });
            ...
            out.salesforce.creditDescribeError = String(e.message || e);
          }
        } else {
        ...
        out.salesforce.tokenError = String(e.message || e);
      }
      try {
        const gt = await getGraphToken(env);
        const GH = { "Authorization": "Bearer " + gt, "Accept": "application/json" };
        ...
        if (!sR.ok) {
          ...
          if (!wr.ok) {
            ...
            const sheets = (wd.value || []).map(function(w) {
              return w.name;
            });
            out.graph.worksheets = sheets;
            const want = (url.searchParams.get("sheet") || "").trim();
            const target = want || sheets.filter(function(n) {
              return /vac|pto/i.test(n);
            })[0] || sheets[0] || null;
            if (target) {
              ...
              else out.graph.peek = { sheet: target, address: rd.address, rows: (rd.values || []).filter(function(row) {
        ...
        out.graph.error = String(e.message || e);
      }
      return json(out, 200, origin);
    }
...
//# sourceMappingURL=cloudworker.js.map
