...
const v1rx = /<script>\/\* ADMIN_PROGRESS_v1 \*\/[\s\S]*?<\/script>/;
if (v1rx.test(h)) {
  h = h.replace(v1rx, V2);
  did.push("script (upgraded v1 -> v2)");
} else {
...
console.log("APPLIED ADMIN_PROGRESS_v2: " + did.join(", "));
